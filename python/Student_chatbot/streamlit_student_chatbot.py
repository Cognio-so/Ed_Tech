...
from Student_AI_tutor import AsyncRAGTutor, RAGTutorConfig
...
if "student_details_submitted" not in st.session_state:
...
async def get_response_stream_async(query: str, history: List[Dict[str, Any]], image_storage_key: Optional[str] = None, uploaded_files: Optional[List[str]] = None, student_details: Optional[Dict[str, Any]] = None):
...
def main():
    ...
    if not st.session_state.student_details_submitted:
        st.header("First, tell me a little about yourself!")
        with st.form("student_details_form"):
            st.write("This will help me personalize your learning experience.")
            name = st.text_input("What is your name?", placeholder="e.g., Alex")
            student_class = st.text_input("What grade/class are you in?", placeholder="e.g., 10th Grade")
            subjects_str = st.text_area("What subjects are you studying? (comma-separated)", placeholder="e.g., Math, Biology, History")
            tasks_str = st.text_area("What are your pending assignments?", placeholder="Format: Subject - Task Details (one per line)\ne.g., Math - Chapter 5 homework\nHistory - Essay on the Renaissance")
            
            submitted = st.form_submit_button("Start Learning!")
            
            if submitted:
                if not all([name, student_class, subjects_str, tasks_str]):
                    st.error("Please fill out all the fields to get started.")
                else:
                    # Parse subjects
        ...
        if not st.session_state.messages and st.session_state.get("student_details"):
            st.info("Our AI Learning Coach is preparing your personalized welcome...")
            initial_query = "Please greet the student and introduce yourself as their learning coach."
